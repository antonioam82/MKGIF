...
def main():
    ...
    parser.add_argument('-ask','--ask_for_coincidents',action='store_true',help='Generates message when finding matches')
    ...
    if args.ask_for_coincidents and args.destination in os.listdir():
        response = confirm(args.destination)
        if response == "Y":
            gm(args)
    else:
        gm(args)
...
def confirm(file):
    confirmation = (input(f"\n'{file}' already exists. Overwrite(Y/n)?: ")).upper()
    while confirmation != "Y" and confirmation != "N":
        confirmation = input(f"'{file}' already exists. Overwrite(Y/n)?: ").upper()
    return confirmation
